...
def get_direction(key, direction):
    if key == curses.KEY_LEFT and direction != "e":
        direction = "w"
    elif key == curses.KEY_RIGHT and direction != "w":
        direction = "e"
    elif key == curses.KEY_UP and direction != "s":
        direction = "n"
...
def boarder_setup(scr, h, w, bottom_boarder, top_boarder, l_boarder, r_boarder, score, level, logo="BOBGAME"):
    scr.clear()
...
def run_game(stdscr):
    ...
    while again == ord("y"):
        ...
        while True:
            ...
            food, food_state, poos, prey, score_add, dead, body = eat(
                food, head, body, food_state, poos, h, w, top_boarder, direction
            )
